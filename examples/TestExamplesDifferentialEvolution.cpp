...
SUITE(ExamplesDifferentialEvolution)
{
TEST(DifferentialEvolution_CardiacAlphaN)  // From Unfit 1
{
  Unfit::DifferentialEvolution de;
  de.bounds.SetBounds(0, -100.0, 100.0);
  de.bounds.SetBounds(1, -100.0, 100.0);
  de.bounds.SetBounds(2, -10.0, 10.0);
  // Constrain DE solution to be within the bounds used to generate the
  // population
  ...
  std::cout << TestTime(t1, t2) << "CardiacAlphaN (DifferentialEvolution)";
  std::cout << std::endl;
...
TEST(DifferentialEvolution_Exponential)
{
  Unfit::DifferentialEvolution de;
  de.bounds.SetBounds(0, 0.0, 100.0);
  ...
  auto rc = de.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Exponential (DifferentialEvolution)";
  std::cout << std::endl;
...
TEST(DifferentialEvolution_Gaussian)
{
  Unfit::DifferentialEvolution de;
  de.bounds.SetBounds(0, 0.0, 100.0);
  de.bounds.SetBounds(1, -100.0, 100.0);
  de.bounds.SetBounds(2, 0.0, 100.0);
  de.bounds.SetBounds(3, -100.0, 100.0);
  de.options.SetRandomSeed(314159);
  // Read in the experimental data
  std::vector<double> x;
  std::vector<double> y;
  Unfit::DataFileReader<double> dfr;
  ...
  auto rc = de.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Gaussian (DifferentialEvolution)";
  std::cout << std::endl;
...
//
// TEST(DifferentialEvolution_GeneticSwitch)  // From Unfit 1
// {
//  Unfit::DifferentialEvolution de;
//  de.bounds.SetBounds(0, 0.0, 1000.0);
//  de.bounds.SetBounds(1, 0.0, 10.0);
//  de.bounds.SetBounds(2, -10.0, 10.0);
//  de.bounds.SetBounds(3, -1.0, 1.0);
//  de.options.SetCrossOver(0.45);
//  de.options.SetGeometricTolerance(0.01);
//
//  // Read in the experimental data
//  std::vector<double> t;
...
//  std::cout << "(DifferentialEvolution)" << std::endl;
//
//  // Check the result matches what we expect
//  CHECK_EQUAL(0, rc);
...
TEST(DifferentialEvolution_HelicalValley)  // From LEVMAR
{
  Unfit::DifferentialEvolution de;
  ...
  auto rc = de.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "HelicalValley ";
  std::cout << "(DifferentialEvolution)" << std::endl;
...
TEST(DifferentialEvolution_HodgkinHuxleyBetaN)  // From Unfit 1
{
  Unfit::DifferentialEvolution de;
  de.bounds.SetBounds(0, -100.0, 100.0);
  de.bounds.SetBounds(1, -100.0, 100.0);
...
TEST(DifferentialEvolution_ModifiedRosenbrock)  // From LEVMAR
{
  Unfit::DifferentialEvolution de;
  de.bounds.SetBounds(0, -10.0, 10.0);
  de.bounds.SetBounds(1, -10.0, 10.0);
...
TEST(DifferentialEvolution_NonStationaryMarkov)  // From Unfit 1
{
  Unfit::DifferentialEvolution de;
  de.bounds.SetBounds(0, -100.0, 100.0);
  de.bounds.SetBounds(1, -100.0, 100.0);
  de.bounds.SetBounds(2, -10.0, 10.0);
  de.bounds.SetBounds(3, 0.0, 10.0);
  // Read in the experimental data
...
TEST(DifferentialEvolution_ODE2D)  // From Unfit 1
{
  Unfit::DifferentialEvolution de;
  de.bounds.SetBounds(0, -100.0, 100.0);
  de.bounds.SetBounds(1, -100.0, 100.0);
  ...
  auto rc = de.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "ODE2D (DifferentialEvolution)";
  std::cout << std::endl;
...
TEST(DifferentialEvolution_ODE3DVariant)  // From Unfit 1
{
  Unfit::DifferentialEvolution de;
  de.bounds.SetBounds(0, -100.0, 100.0);
  de.bounds.SetBounds(1, -100.0, 100.0);
  ...
  auto rc = de.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "ODE3DVariant (DifferentialEvolution)"
      << std::endl;
...
TEST(DifferentialEvolution_Osborne)  // From LEVMAR
{
  Unfit::DifferentialEvolution de;
  de.bounds.SetBounds(0, -10.0, 10.0);
  de.bounds.SetBounds(1, -10.0, 10.0);
  de.bounds.SetBounds(2, -10.0, 10.0);
  de.bounds.SetBounds(3, -10.0, 10.0);
  de.bounds.SetBounds(4, -10.0, 10.0);
  ...
  auto rc = de.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Osborne (DifferentialEvolution)";
  std::cout << std::endl;
...
TEST(DifferentialEvolution_Parabolic)
{
  Unfit::DifferentialEvolution de;
  de.bounds.SetBounds(0, -100.0, 100.0);
  de.bounds.SetBounds(1, -100.0, 100.0);
  de.bounds.SetBounds(2, -100.0, 100.0);
  // Read in the experimental data
  std::vector<double> x;
  std::vector<double> y;
  Unfit::DataFileReader<double> dfr;
  ...
  auto rc = de.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Parabolic (DifferentialEvolution)";
  std::cout << std::endl;
...
TEST(DifferentialEvolution_Powell)  // From LEVMAR
{
  Unfit::DifferentialEvolution de;
  de.bounds.SetBounds(0, -100.0, 100.0);
  de.bounds.SetBounds(1, -100.0, 100.0);
  ...
  auto rc = de.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Powell (DifferentialEvolution)";
  std::cout << std::endl;
...
TEST(DifferentialEvolution_Rosenbrock)  // From LEVMAR
{
  Unfit::DifferentialEvolution de;
  de.bounds.SetBounds(0, -10.0, 10.0);
  de.bounds.SetBounds(1, -10.0, 10.0);
  ...
  auto rc = de.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Rosenbrock (DifferentialEvolution)";
  std::cout << std::endl;
...
TEST(DifferentialEvolution_ThreeNaMarkov)  // From Unfit 1
{
  Unfit::DifferentialEvolution de;
  de.bounds.SetBounds(0, 0.0, 10.0);
  de.bounds.SetBounds(1, 0.0, 10.0);
  de.bounds.SetBounds(2, 0.0, 10.0);
  de.bounds.SetBounds(3, 0.0, 10.0);
  // Read in the experimental data
  std::vector<double> t;
  ...
  auto rc = de.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "ThreeNaMarkov (DifferentialEvolution)";
  std::cout << std::endl;
...
TEST(DifferentialEvolution_Woods)  // From LEVMAR
{
  Unfit::DifferentialEvolution de;
  de.bounds.SetBounds(0, -10.0, 10.0);
  de.bounds.SetBounds(1, -10.0, 10.0);
  ...
  auto rc = de.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Woods (DifferentialEvolution)";
  std::cout << std::endl;
