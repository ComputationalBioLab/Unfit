...
SUITE(ExamplesParticleSwarm)
{
TEST(ParticleSwarm_CardiacAlphaN)  // From Unfit 1
{
  Unfit::ParticleSwarm ps;
  ps.bounds.SetBounds(0, -100.0, 100.0);
  ps.bounds.SetBounds(1, -100.0, 100.0);
  ps.bounds.SetBounds(2, -10.0, 10.0);
  ...
  std::cout << TestTime(t1, t2) << "CardiacAlphaN (ParticleSwarm)";
  std::cout << std::endl;
...
TEST(ParticleSwarm_Exponential)
{
  Unfit::ParticleSwarm ps;
  ps.bounds.SetBounds(0, 0.0, 100.0);
  ps.bounds.SetBounds(1, -10.0, 10.0);
  ps.bounds.SetBounds(2, -10.0, 10.0);
  ps.options.SetUseAdaptiveParameters(true);
  ps.options.SetStrategy(6);
  ps.options.SetPopulationSize(100);
  ps.options.SetMaxFunctionEvaluations(500000);
  // Read in the experimental data
  std::vector<double> x;
  std::vector<double> y;
  Unfit::DataFileReader<double> dfr;
  ...
  auto rc = ps.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Exponential (ParticleSwarm)";
  std::cout << std::endl;

  // Check the result matches what we expect
  CHECK_EQUAL(0, rc);
  CHECK_CLOSE(3.66696, min_point[0], 0.15);
  CHECK_CLOSE(0.0885905, min_point[1], 0.01);
  CHECK_CLOSE(3.06235, min_point[2], 0.01);
}

TEST(ParticleSwarm_Gaussian)
{
  Unfit::ParticleSwarm ps;
  ps.bounds.SetBounds(0, 0.0, 100.0);
  ps.bounds.SetBounds(1, 0.0, 100.0);
  ps.bounds.SetBounds(2, 0.0, 100.0);
  ps.bounds.SetBounds(3, 0.0, 100.0);
  ps.options.SetAlpha(0.5);
  // Read in the experimental data
  std::vector<double> x;
  std::vector<double> y;
  Unfit::DataFileReader<double> dfr;
  ...
  auto rc = ps.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Gaussian (ParticleSwarm)";
  std::cout << std::endl;
...
//
// TEST(ParticleSwarm_GeneticSwitch)  // From Unfit 1
// {
//  Unfit::ParticleSwarm ps;
//  ps.bounds.SetBounds(0, 0.0, 1000.0);
//  ps.bounds.SetBounds(1, 0.0, 10.0);
//  ps.bounds.SetBounds(2, -10.0, 10.0);
//  ps.bounds.SetBounds(3, -1.0, 1.0);
//  ps.options.SetMaxFunctionEvaluations(200000);
//  // Read in the experimental data
//  std::vector<double> t;
...
//  auto rc = ps.FindMin(cost_func, min_point);
//  auto t2 = hrclock_t::now();  // End time
//  std::cout << TestTime(t1, t2) << "GeneticSwitch (ParticleSwarm)";
//  std::cout << std::endl;
//
//  // Check the result matches what we expect
//  CHECK_EQUAL(0, rc);
//  CHECK_CLOSE(216.433109901728, min_point[0], 10.0);
//  CHECK_CLOSE(2.00476590272927, min_point[1], 0.1);
//  CHECK_CLOSE(-2.17034564257086, min_point[2], 0.1);
//  CHECK_CLOSE(-0.226712906915202, min_point[3], 0.1);
// }

TEST(ParticleSwarm_HelicalValley)  // From LEVMAR
{
  Unfit::ParticleSwarm ps;
  ...
  auto rc = ps.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "HelicalValley (ParticleSwarm)";
  std::cout << std::endl;
...
TEST(ParticleSwarm_HodgkinHuxleyBetaN)  // From Unfit 1
{
  // alpha is set as 1.0, beta is 0.2 and delta is 0.99
  Unfit::ParticleSwarm ps;
  ps.bounds.SetBounds(0, -100.0, 100.0);
  ps.bounds.SetBounds(1, -100.0, 100.0);
  // Read in the experimental data
  ...
  auto rc = ps.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "HodgkinHuxleyBetaN (ParticleSwarm)";
  std::cout << std::endl;
...
TEST(ParticleSwarm_ModifiedRosenbrock)  // From LEVMAR
{
  Unfit::ParticleSwarm ps;
  ps.bounds.SetBounds(0, -10.0, 10.0);
  ps.bounds.SetBounds(1, -10.0, 10.0);
  ...
  auto rc = ps.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "ModifiedRosenbrock (ParticleSwarm)";
  std::cout << std::endl;
...
TEST(ParticleSwarm_NonStationaryMarkov)  // From Unfit 1
{
  Unfit::ParticleSwarm ps;
  ps.bounds.SetBounds(0, -100.0, 100.0);
  ps.bounds.SetBounds(1, -100.0, 100.0);
  ps.bounds.SetBounds(2, -10.0, 10.0);
  ps.bounds.SetBounds(3, 0.0, 10.0);
  // Read in the experimental data
  ...
  auto rc = ps.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "NonstationaryMarkov (ParticleSwarm)";
  std::cout << std::endl;
...
TEST(ParticleSwarm_ODE2D)  // From Unfit 1
{
  Unfit::ParticleSwarm ps;
  ps.bounds.SetBounds(0, -100.0, 100.0);
  ps.bounds.SetBounds(1, -100.0, 100.0);
  ...
  auto rc = ps.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "ODE2D (ParticleSwarm)";
  std::cout << std::endl;
...
TEST(ParticleSwarm_ODE3DVariant)  // From Unfit 1
{
  Unfit::ParticleSwarm ps;
  ps.bounds.SetBounds(0, -100.0, 100.0);
  ps.bounds.SetBounds(1, -100.0, 100.0);
  ...
  auto rc = ps.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "ODE3DVariant (ParticleSwarm)";
  std::cout << std::endl;

  // Check the result matches what we expect
  CHECK_EQUAL(0, rc);
  CHECK_CLOSE(2.21403, min_point[0], 0.01);
  CHECK_CLOSE(3.32104, min_point[1], 0.3);
  CHECK_CLOSE(0.0, min_point[2], 0.1);
}

TEST(ParticleSwarm_Osborne)  // From LEVMAR
{
  Unfit::ParticleSwarm ps;
  ps.bounds.SetBounds(0, 0.0, 1.0);
  ps.bounds.SetBounds(1, 1.0, 2.0);
  ps.bounds.SetBounds(2, -1.5, -1.0);
  ps.bounds.SetBounds(3, 0.0, 1.0);
  ps.bounds.SetBounds(4, 0.0, 1.0);
  ps.options.SetUseHardBounds(true);
  ...
  auto rc = ps.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Osborne (ParticleSwarm)";
  std::cout << std::endl;

  // Check the result matches what we expect
  CHECK_EQUAL(0, rc);
  CHECK_CLOSE(0.3754, min_point[0], 5e-3);
  CHECK_CLOSE(1.9358, min_point[1], 2e-1);
  CHECK_CLOSE(-1.4647, min_point[2], 2e-1);
  CHECK_CLOSE(0.0129, min_point[3], 1e-3);
  CHECK_CLOSE(0.0221, min_point[4], 1e-3);
}

TEST(ParticleSwarm_Parabolic)
{
  Unfit::ParticleSwarm ps;
  ps.bounds.SetBounds(0, -100.0, 0.0);
  ps.bounds.SetBounds(1, 0.0, 100.0);
  ps.bounds.SetBounds(2, -10.0, 10.0);
  ps.options.SetUseHardBounds(true);
  // Read in the experimental data
  ...
  auto rc = ps.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Parabolic (ParticleSwarm)";
  std::cout << std::endl;
...
TEST(ParticleSwarm_Powell)  // From LEVMAR
{
  Unfit::ParticleSwarm ps;
  ps.bounds.SetBounds(0, -100.0, 100.0);
  ps.bounds.SetBounds(1, -100.0, 100.0);
  ...
  auto rc = ps.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Powell (ParticleSwarm)";
  std::cout << std::endl;
...
TEST(ParticleSwarm_Rosenbrock)  // From LEVMAR
{
  Unfit::ParticleSwarm ps;
  ps.bounds.SetBounds(0, -10.0, 10.0);
  ps.bounds.SetBounds(1, -10.0, 10.0);
  ...
  auto rc = ps.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Rosenbrock (ParticleSwarm)";
  std::cout << std::endl;
...
TEST(ParticleSwarm_ThreeNaMarkov)  // From Unfit 1
{
  Unfit::ParticleSwarm ps;
  ps.bounds.SetBounds(0, 0.0, 10.0);
  ps.bounds.SetBounds(1, 0.0, 10.0);
  ps.bounds.SetBounds(2, 0.0, 10.0);
  ps.bounds.SetBounds(3, 0.0, 10.0);
  ps.options.SetAlpha(0.5);
  // Read in the experimental data
  std::vector<double> t;
  ...
  auto rc = ps.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "ThreeNaMarkov (ParticleSwarm)";
  std::cout << std::endl;

  // Check the result matches what we expect
  CHECK_EQUAL(0, rc);
  CHECK_CLOSE(0.0494253, min_point[0], 1e-2);
  CHECK_CLOSE(0.0283517, min_point[1], 1e-2);
  CHECK_CLOSE(0.321102, min_point[2], 1e-2);
  CHECK_CLOSE(0.00174363, min_point[3], 1e-4);
}

TEST(ParticleSwarm_Woods)  // From LEVMAR
{
  Unfit::ParticleSwarm ps;
  ps.bounds.SetBounds(0, -10.0, 10.0);
  ps.bounds.SetBounds(1, -10.0, 10.0);
  ps.bounds.SetBounds(2, -10.0, 10.0);
  ps.bounds.SetBounds(3, -10.0, 10.0);
  ps.options.SetUseHardBounds(true);
  ps.options.SetPopulationSize(100);
  ps.options.SetMaxFunctionEvaluations(500000);
  // Create the cost function
  ...
  auto rc = ps.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Woods (ParticleSwarm)";
  std::cout << std::endl;

  // Check the result matches what we expect
  CHECK_EQUAL(0, rc);
  CHECK_CLOSE(1, min_point[0], 0.1);
  CHECK_CLOSE(1, min_point[1], 0.1);
  CHECK_CLOSE(1, min_point[2], 0.1);
  CHECK_CLOSE(1, min_point[3], 0.1);
}
}  // suite ExamplesParticleSwarm
