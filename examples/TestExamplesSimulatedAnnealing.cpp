...
SUITE(ExamplesSimulatedAnnealing)
{
TEST(SimulatedAnnealing_CardiacAlphaN)  // From Unfit 1
{
  Unfit::SimulatedAnnealing sa;
  sa.bounds.SetBounds(0, -100.0, 100.0);
  ...
  std::cout << TestTime(t1, t2) << "CardiacAlphaN (SimulatedAnnealing)";
  std::cout << std::endl;

  // Check the result matches what we expect
  CHECK_EQUAL(0, rc);
  CHECK_CLOSE(0.0731685, min_point[0], 1e-3);
  CHECK_CLOSE(-0.033983, min_point[1], 1e-3);
  CHECK_CLOSE(3.10956, min_point[2], 1e-3);
}

TEST(SimulatedAnnealing_Exponential)
{
  Unfit::SimulatedAnnealing sa;
  sa.bounds.SetBounds(0, 0.0, 100.0);
  ...
  auto rc = sa.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Exponential (SimulatedAnnealing)";
  std::cout << std::endl;
...
TEST(SimulatedAnnealing_Gaussian)
{
  Unfit::SimulatedAnnealing sa;
  sa.bounds.SetBounds(0, 0.0, 100.0);
  sa.bounds.SetBounds(1, -100.0, 100.0);
  sa.bounds.SetBounds(2, 0.0, 100.0);
  sa.bounds.SetBounds(3, -100.0, 100.0);
  // Read in the experimental data
  std::vector<double> x;
  std::vector<double> y;
  Unfit::DataFileReader<double> dfr;
  ...
  auto rc = sa.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Gaussian (SimulatedAnnealing)";
  std::cout << std::endl;
  ...
  CHECK_CLOSE(4.0337, fabs(min_point[2]), 1e-4);
...
//
// TEST(SimulatedAnnealing_GeneticSwitch)  // From Unfit 1
// {
//  Unfit::SimulatedAnnealing sa;
//  sa.bounds.SetBounds(0, 100.0, 250.0);
//  sa.bounds.SetBounds(1, 0.0, 10.0);
//  sa.bounds.SetBounds(2, -10.0, 0.0);
//  sa.bounds.SetBounds(3, -1.0, 0.0);
//  sa.options.SetNumberOfTemperatureLoops(10);
//  sa.options.SetTemperature(10000.0);
...
//  auto rc = sa.FindMin(cost_func, min_point);
//  auto t2 = hrclock_t::now();  // End time
//  std::cout << TestTime(t1, t2) << "GeneticSwitch (SimulatedAnnealing)";
//  std::cout << std::endl;
//
//  // Check the result matches what we expect
//  CHECK_EQUAL(0, rc);
//  CHECK_CLOSE(216.433109901728, min_point[0], 1e-2);
...
TEST(SimulatedAnnealing_HelicalValley)  // From LEVMAR
{
  Unfit::SimulatedAnnealing sa;
  ...
  auto rc = sa.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "HelicalValley (SimulatedAnnealing)";
  std::cout << std::endl;
...
TEST(SimulatedAnnealing_HodgkinHuxleyBetaN)  // From Unfit 1
{
  Unfit::SimulatedAnnealing sa;
  sa.bounds.SetBounds(0, -100.0, 100.0);
  sa.bounds.SetBounds(1, -100.0, 100.0);
  ...
  auto rc = sa.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "HodgkinHuxleyBetaN (SimulatedAnnealing)";
  std::cout << std::endl;

  // Check the result matches what we expect
  CHECK_EQUAL(0, rc);
  CHECK_CLOSE(0.118753, min_point[0], 1e-3);
  CHECK_CLOSE(-91.5825, min_point[1], 1e-2);
}

TEST(SimulatedAnnealing_ModifiedRosenbrock)  // From LEVMAR
{
  Unfit::SimulatedAnnealing sa;
  sa.bounds.SetBounds(0, -10.0, 10.0);
  sa.bounds.SetBounds(1, -10.0, 10.0);
  ...
  auto rc = sa.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "ModifiedRosenbrock (SimulatedAnnealing)";
  std::cout << std::endl;
...
TEST(SimulatedAnnealing_NonStationaryMarkov)  // From Unfit 1
{
  Unfit::SimulatedAnnealing sa;
  sa.bounds.SetBounds(0, -100.0, 100.0);
  sa.bounds.SetBounds(1, -100.0, 100.0);
  sa.bounds.SetBounds(2, -10.0, 10.0);
  sa.bounds.SetBounds(3, 0.0, 10.0);
  // Read in the experimental data
  ...
  auto rc = sa.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "NonstationaryMarkov (SimulatedAnnealing)";
  std::cout << std::endl;

  // Check the result matches what we expect
  CHECK_EQUAL(0, rc);
  CHECK_CLOSE(21.243, min_point[0], 1e-3);
  CHECK_CLOSE(23.826, min_point[1], 1e-2);
  CHECK_CLOSE(0.29907, min_point[2], 1e-3);
  CHECK_CLOSE(0.18659, fabs(min_point[3]), 1e-3);
}


TEST(SimulatedAnnealing_ODE2D)  // From Unfit 1
{
  Unfit::SimulatedAnnealing sa;
  sa.bounds.SetBounds(0, -100.0, 100.0);
  sa.bounds.SetBounds(1, -100.0, 100.0);
  ...
  auto rc = sa.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "ODE2D (SimulatedAnnealing)";
  std::cout << std::endl;
...
TEST(SimulatedAnnealing_ODE3DVariant)  // From Unfit 1
{
  Unfit::SimulatedAnnealing sa;
  sa.bounds.SetBounds(0, -100.0, 100.0);
  sa.bounds.SetBounds(1, -100.0, 100.0);
  ...
  auto rc = sa.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "ODE3DVariant (SimulatedAnnealing)";
  std::cout << std::endl;
...
// MB: This test example does run, but it takes a long time and so has
//     been omitted from the regular tests for now. The bounds are also
//     very tight to the answer. Not sure why SA struggles with this one,
//     perhaps because the solution space is pretty flat near the minimum.
//
// TEST(SimulatedAnnealing_Osborne)  // From LEVMAR
// {
//  Unfit::SimulatedAnnealing sa;
//  sa.bounds.SetBounds(0, 0.0, 0.5);
//  sa.bounds.SetBounds(1, -2.0, 2.0);
//  sa.bounds.SetBounds(2, -2.0, 2.0);
//  sa.bounds.SetBounds(3, 0.0, 0.1);
//  sa.bounds.SetBounds(4, 0.0, 0.1);
//  sa.options.SetNumberOfTemperatureLoops(200);
//  sa.options.SetMaxFunctionEvaluations(7500000);
...
//  auto rc = sa.FindMin(cost_func, min_point);
//  auto t2 = hrclock_t::now();  // End time
//  std::cout << TestTime(t1, t2) << "Osborne (SimulatedAnnealing)";
//  std::cout << std::endl;
//
//  // Check the result matches what we expect
//  CHECK_EQUAL(0, rc);
//  CHECK_CLOSE(0.3754, min_point[0], 1e-3);
//  // Note there is the possibility of [1] & [3] swapping with [2] & [4] and
//  // the solution is still valid as we are fitting two exponentials
//  if (min_point[1] > min_point[2]) {
//    CHECK_CLOSE(1.9358, min_point[1], 1e-2);
//    CHECK_CLOSE(-1.4647, min_point[2], 1e-2);
//    CHECK_CLOSE(0.0129, min_point[3], 1e-4);
//    CHECK_CLOSE(0.0221, min_point[4], 1e-4);
//  }
//  else {
//    CHECK_CLOSE(1.9358, min_point[2], 1e-2);
//    CHECK_CLOSE(-1.4647, min_point[1], 1e-2);
//    CHECK_CLOSE(0.0129, min_point[4], 1e-4);
//    CHECK_CLOSE(0.0221, min_point[3], 1e-4);
//  }
// }

TEST(SimulatedAnnealing_Parabolic)
{
  Unfit::SimulatedAnnealing sa;
  sa.bounds.SetBounds(0, -100.0, 100.0);
  sa.bounds.SetBounds(1, -100.0, 100.0);
  sa.bounds.SetBounds(2, -100.0, 100.0);
  // Read in the experimental data
  std::vector<double> x;
  std::vector<double> y;
  Unfit::DataFileReader<double> dfr;
  ...
  auto rc = sa.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Parabolic (SimulatedAnnealing)";
  std::cout << std::endl;
...
TEST(SimulatedAnnealing_Powell)  // From LEVMAR
{
  Unfit::SimulatedAnnealing sa;
  sa.bounds.SetBounds(0, -100.0, 100.0);
  sa.bounds.SetBounds(1, -100.0, 100.0);
  sa.options.SetTemperature(2000.0);
  sa.options.SetNumberOfTemperatureLoops(10);
  sa.options.SetMaxFunctionEvaluations(200000);
  // Create the cost function
  ...
  auto rc = sa.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Powell (SimulatedAnnealing)";
  std::cout << std::endl;
...
TEST(SimulatedAnnealing_Rosenbrock)  // From LEVMAR
{
  Unfit::SimulatedAnnealing sa;
  sa.bounds.SetBounds(0, -10.0, 10.0);
  sa.bounds.SetBounds(1, -10.0, 10.0);
  ...
  auto rc = sa.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Rosenbrock (SimulatedAnnealing)";
  std::cout << std::endl;
...
TEST(SimulatedAnnealing_ThreeNaMarkov)  // From Unfit 1
{
  Unfit::SimulatedAnnealing sa;
  sa.bounds.SetBounds(0, 0.0, 10.0);
  sa.bounds.SetBounds(1, 0.0, 10.0);
  sa.bounds.SetBounds(2, 0.0, 10.0);
  sa.bounds.SetBounds(3, 0.0, 10.0);
  // Read in the experimental data
  std::vector<double> t;
  ...
  auto rc = sa.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "ThreeNaMarkov (SimulatedAnnealing)";
  std::cout << std::endl;

  // Check the result matches what we expect
  CHECK_EQUAL(0, rc);
  CHECK_CLOSE(0.0494253, min_point[0], 1e-4);
  CHECK_CLOSE(0.0283517, min_point[1], 1e-4);
  CHECK_CLOSE(0.321102, min_point[2], 1e-4);
  CHECK_CLOSE(0.00174363, min_point[3], 1e-4);
}

TEST(SimulatedAnnealing_Woods)  // From LEVMAR
{
  Unfit::SimulatedAnnealing sa;
  sa.bounds.SetBounds(0, -10.0, 10.0);
  sa.bounds.SetBounds(1, -10.0, 10.0);
  sa.bounds.SetBounds(2, -10.0, 10.0);
  sa.bounds.SetBounds(3, -10.0, 10.0);
  // Create the cost function
  ...
  auto rc = sa.FindMin(cost_func, min_point);
  auto t2 = hrclock_t::now();  // End time
  std::cout << TestTime(t1, t2) << "Woods (SimulatedAnnealing)";
  std::cout << std::endl;
